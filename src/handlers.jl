...
function clear_runtime_plots!(app_run)
    ...
    app_run.histogram_ch1[] = fill(NaN, n_hist)
    app_run.fit_ch1[] = fill(NaN, n_hist)
    app_run.counts_ch1[] = 0.0
    app_run.histogram_ch2[] = fill(NaN, n_hist)
    app_run.fit_ch2[] = fill(NaN, n_hist)
    app_run.counts_ch2[] = 0.0
    app_run.i[] = 0
    app_run.save_progress[] = NaN

    notify(app_run.histogram_ch1)
    notify(app_run.fit_ch1)
    ...
    notify(app_run.histogram_ch2)
    notify(app_run.fit_ch2)
    ...
    notify(app_run.counts_ch1)
    notify(app_run.counts_ch2)
    notify(app_run.i)
    return nothing
end
