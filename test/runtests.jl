...
@testset "protocol schedule math" begin
    ...
    @test isnan(FLIMApp.protocol_setpoint_at_timestamp(protocol, 2.0))
    # First step [5, 15), second step [15, 35)
    @test FLIMApp.protocol_setpoint_at_timestamp(protocol, 6.0) == 3.5
    @test FLIMApp.protocol_setpoint_at_timestamp(protocol, 20.0) == 4.0
    # Second repeat of the 30 s cycle: t = 5 + 30 + 2 is inside step 1 again
    @test FLIMApp.protocol_setpoint_at_timestamp(protocol, 37.0) == 3.5
    # After both repeats (5 + 2*30 = 65): schedule over
    @test isnan(FLIMApp.protocol_setpoint_at_timestamp(protocol, 70.0))
    # Non-finite timestamp
    @test isnan(FLIMApp.protocol_setpoint_at_timestamp(protocol, NaN))
    ...
    @test FLIMApp.protocol_setpoint_at_timestamp(forever, 1234.0) == 2.5
...
@testset "smoothing" begin
    ...
    @test FLIMApp.smooth_strength_factor(1) == 1.0
    @test FLIMApp.smooth_strength_factor(10) == 10.0

    values = [3.0, 3.1, 2.9, 3.05, 3.0]
    # Level 0 is an exact passthrough
    @test FLIMApp.compute_lifetime_smooth_at(values, 3, 0, 3.0) == values[3]
    # Out-of-bounds index and NaN input give NaN
    @test isnan(FLIMApp.compute_lifetime_smooth_at(values, 0, 3, 3.0))
    @test isnan(FLIMApp.compute_lifetime_smooth_at([NaN], 1, 3, 3.0))
    # Smoothing pulls the estimate between prev_smooth and the raw value
    smoothed = FLIMApp.compute_lifetime_smooth_at(values, 2, 5, 3.0)
    @test 3.0 <= smoothed <= values[2]

    # PID Kalman: level 0 passes raw lifetime through
    lifetime_for_pid, prev_smooth, prev_raw, scale = FLIMApp.update_pid_lifetime_kalman(3.2, NaN, NaN, NaN, 0)
    @test lifetime_for_pid == 3.2
    @test prev_smooth == 3.2 && prev_raw == 3.2 && scale > 0
    # Non-finite lifetime is returned unchanged
    nan_result = FLIMApp.update_pid_lifetime_kalman(NaN, 3.0, 3.0, 0.1, 5)
    @test isnan(nan_result[1])
end
...
@testset "plot windowing helpers" begin
    ...
    @test FLIMApp.normalized_counts_for_histogram([2.0, 4.0], [1.0, 8.0]) == [0.25, 0.5]
end

@testset "ChannelSeries / AppRun runtime state" begin
    app = AppState(true)
    app_run = AppRun()
    ...
    FLIMApp.accumulate_channel_sample!(app, app_run.ch1, frame)
    FLIMApp.accumulate_channel_sample!(app, app_run.ch2, ChannelFrame())

    @test app_run.ch1.photons[] == [100.0]
    @test app_run.ch1.lifetime[] == [3.0]
    @test app_run.ch1.lifetime_smooth[] == [3.0]  # level 0: passthrough
    @test app_run.ch1.concentration[] == [1.5]
    @test isnan(app_run.ch2.lifetime[][1])        # absent-channel sentinel

    FLIMApp.publish_channel_frame!(app_run.ch1, frame)
    @test app_run.ch1.counts[] == 100.0
    @test app_run.ch1.histogram[] == [1.0, 2.0]

    FLIMApp.reset_acquisition_observables!(app_run)
    @test isempty(app_run.ch1.photons[])
    @test isempty(app_run.ch2.lifetime[])
    @test app_run.ch1.counts[] == 0.0
    @test isnan(app_run.save_progress[])

    FLIMApp.clear_runtime_plots!(app_run)
    n_hist = length(app_run.hist_time[])
    @test length(app_run.ch1.histogram[]) == n_hist
    @test all(isnan, app_run.ch1.histogram[])
    @test all(isnan, app_run.ch2.fit[])
end

@testset "PID command" begin
    ...
    @test isnan(FLIMApp.pid_command_from_state(state, 4.0, 1.0, 1.0, 1.0, false, false))
    @test isnan(FLIMApp.pid_command_from_state(state, NaN, 1.0, 1.0, 1.0, false, true))
    # P*1 + I*2 + D*0.5 = 3.5
    @test FLIMApp.pid_command_from_state(state, 4.0, 1.0, 1.0, 1.0, false, true) == 3.5
    # inverted and clamped to [0, 100]
    @test FLIMApp.pid_command_from_state(state, 4.0, 1.0, 1.0, 1.0, true, true) == 0.0
    @test FLIMApp.pid_command_from_state(state, 4.0, 100.0, 100.0, 100.0, false, true) == 100.0
end

@testset "acquisition helpers" begin
    @test FLIMApp.initial_guess_for_lifetime_count("1 lifetime") == [3.0, 0.0, 5.0e-5]
    @test length(FLIMApp.initial_guess_for_lifetime_count("2 lifetimes")) == 5
    @test length(FLIMApp.initial_guess_for_lifetime_count("3 lifetimes")) == 7
